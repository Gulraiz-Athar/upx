...
class ShipController extends Controller
{
                ...
                if ($service_id == 1) {
                    ...
                    if (!empty($zone)) {
                        ...
                        if (!empty($selectedweight)) {
                            ...
                            // return $zoneid.  "servicetype" . $servicetype . "agentprice" . $agentprice. "handling" . $handling_price_agent;
                            
                            // return $servicetype . "weightid" .  $weightid . "zoneid" . $zoneid . "userid" . Auth::user()->id . "servicetype". $servicetype; 
                    ...
                    if ($input['booking_status'] == 0) {
                        ...
                        if ($error) {
                            ...
                            /***************************************** Booking store  ****************************************/
                            $final_total_upx = $finalprice - abs($input['discount_amt']) + $input['packing_charge_amt'] + $input['tax_amt'];
                            $final_total_agent = $finalagentprice;

                            if ($finalagentprice > 0) {
                                if ($finalagentprice >= abs($input['discount_amt'])) {
                                    $final_total_agent = $finalagentprice - abs($input['discount_amt']) + $input['packing_charge_amt'] + $input['tax_amt'];
                                } else {
                            ...
                            $bookingid = Booking::create([
                                ...
                                'discount_amt' => abs($input['discount_amt']),
                            ...
                            $this->StoreSenderReceiverAddress(Auth::id(), $inputdata);
                            ...
                            // $send_msg = send_sms($booking->tracking_number,$input['first_name_s'], $final_total_upx,$input['phone_s'],$input['coutry_s']);
                            $arrs = [];
                            // if($send_msg['status'] == 411 ) {
                            //          $arrs = array("status" => 401, "msga" => 'update can’t be sent as the number is not registered with WhatsApp.', "result" => array(), 'bookingid' => $bookingid, 'boxids' => $boxids);

                            // }

                            commit();
                            $arr = array("status" => 200, "msg" => $msg, 'bookingid' => $bookingid, 'boxids' => $boxids,"arrs" => $arrs);
                        }
                    }
                } else if($service_id == 2){ 
                    foreach ($input['length'] as $key => $value) {
    ...
    public function quotation_modal(Request $request)
    {
       ...
       public function StoreSenderReceiverAddress($userid, $input)
    {
        $book = AddressBook::where([['created_by',$userid],['type','Sender'],['address1',$input['address1_s']],['name',$input['first_name_s']]])->first();
            if(!empty($book)){
            ...
            $addresssender->created_by =  $userid;
            $addresssender->type =  'Sender';
            $addresssender->name =  $input['first_name_s'];
            $addresssender->email =  $input['email_s'];
            $addresssender->address1 =  $input['address1_s'];
            $addresssender->address2 =  $input['address2_s'];
            $addresssender->address3 =  $input['address3_s'];
            $addresssender->country_id =  $input['coutry_s'];
            $addresssender->city =  $input['city_s'];
            $addresssender->state =  $input['state_s'];
            $addresssender->postalcode =  $input['postal_code_s'];
            $addresssender->phone_number =  $input['phone_s'];
            $addresssender->company =  $input['company_s'];
            $addresssender->save();


            $bookr = AddressBook::where([['created_by',$userid],['type','Receiver'],['company',$input['company_r']],['address1',$input['address1_r']],['name',$input['full_name_r']]])->first();
            if(!empty($bookr)){
            ...
            $addressreceiver->created_by =  $userid;
            $addressreceiver->type =  'Receiver';
            $addressreceiver->name =  $input['full_name_r'];
            $addressreceiver->email =  $input['email_r'];
            $addressreceiver->address1 =  $input['address1_r'];
            $addressreceiver->address2 =  $input['address2_r'];
            $addressreceiver->address3 =  $input['address3_r'];
            $addressreceiver->country_id =  $input['country_r'];
            $addressreceiver->city =  $input['city_r'];
            $addressreceiver->state =  $input['state_r'];
            $addressreceiver->postalcode =  $input['postal_code_r'];
            $addressreceiver->phone_number =  $input['phone_r'];
            $addressreceiver->company =  $input['company_r'];
