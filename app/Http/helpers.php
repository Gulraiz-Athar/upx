...
function generateTrackNumber(){
    ...
    if($user){
        $bookingcount = Booking::count()+1;
        // $code_number = ($user->code_number)?'786'.$user->code_number:'786'.rand(100,999);

        //$bookingcount = BookingDimension::where('booking_id',$booking_id)->count()+1;
        $boxnumber= '140'.str_pad($bookingcount, 7, '0', STR_PAD_LEFT);
    }
   /* $n = 7;
    $characters = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $randomString = '';
...
function send_sms($tracking_number,$name,$final_total_upx,$phonenumber,$country_id) {

    $code = Country::where('id',$country_id)->first();
    $c_code = $code->country_isd_code;
    if($c_code == '' && empty($c_code) && !isset($c_code)){
        $c_code = '+44';
    }
    $ct_code = $c_code.$phonenumber;
   //$wh = 'whatsapp:'.$ct_code.'';
    // echo $ct_code;
    // echo $wh;
    // exit;
    ...
    try
        ...
        //          'body' => 'Hi '.$name.', we received payment of £'.$final_total_upx.' for your consignment booking with UPX. Your tracking number is '.$tracking_number.'. You will get updates regarding the shipping and delivery by SMSs from us.Thanks!'
        //     )
        //  );
            // $country_code = '91';
            // $mobile = '7567434957';
            ...
            // the number you'd like to send the message to
            // 'whatsapp:+917567434957',
                //"'whatsapp:".$ct_code."',
                //'whatsapp:'.$code->country_isd_code.''.$phonenumber.'',
                "whatsapp:$ct_code",
            array(
                 // A Twilio phone number you purchased at twilio.com/console
           ...
           return $arr = array("status" => 200, "msg" => 'success');
        }
       ...
       catch (Exception $ex)
        {
            ...
            return array("status" => 411, "msg" => $msg, "result" => array());
        }
}

function send_sms_status($tracking_num,$logid,$name,$phonenumber,$country_code) {
    ...
    if ($logid == 1) {
        $msg = 'Hi '.$name.', your consignment is out for shipped and will be delivered soon today. Its on its way to the destination.';
    } elseif ($logid == 2 ) {
        $msg = 'Hi '.$name.', your consignment is out for collected and will be delivered soon today. Its on its way to the destination.';
    } elseif ($logid == 3 ) {
    ...
    try
        ...
        $ct_code = $country_code.$phonenumber;


        $message = $client->messages->create(
            // the number you'd like to send the message to
                // 'whatsapp:+917567434957',
                //'whatsapp:'.$country_code.''.$phonenumber,
                "whatsapp:$ct_code",
            array(
                 // A Twilio phone number you purchased at twilio.com/console
                 'from' => 'whatsapp:+14155238886',
       ...
       catch (Exception $ex)
        {
            ...
            return array("status" => 400, "msg" => $msg, "result" => array());
        }
}
?>
