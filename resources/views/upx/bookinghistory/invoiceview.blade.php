...
<section class="content">
    ...
    <section class="invoice" style="margin: 0px;">
            ...
            <div class="col-sm-4 invoice-col">
                ...
                <address>
                    @if(!empty($booking->addressesreceiver->company))
                        <strong>Company: </strong>{{ $booking->addressesreceiver->company }}<br>
                    @endif
                    <strong> Name: </strong>{{ $booking->addressesreceiver->name }} {{ $booking->addressesreceiver->lastname }}<br>
                    ...
                    @if(!empty($booking->dimentions))
                        @foreach($booking->dimentions as $dimention)
                            <tr>
                                ...
                                <td>1</td>
                                <td>{{ $dimention->lenth }}Cm</td>
                                <td>{{ $dimention->width }}Cm</td>
                                <td>{{ $dimention->height }}Cm</td>
                                <td>{{ $dimention->weight }}Kg</td>
                                <td>&#163; {{ $dimention->consignment_amt ?? '0.00' }}</td>
                                <td>&#163; {{ $dimention->insure_amt ?? '0.00' }}</td>
        ...
        <div class="row">
            ...
            <div class="col-xs-2">
                <p class="lead"><i class="fa fa-info-circle" aria-hidden="true"></i> Service Info:</p>
                <p><b>Service: </b> {{ $booking->service->name }}</p>

                @if($booking->service_type)
                    <p><b>Service Type: </b> {{ ucwords($booking->service_type) }}</p>
                @endif
                @if($timeline_data)
                    <p><b>Transit Time: </b> {{ ucwords($timeline_data) }}</p>
                @endif

                <p><b>Payment Method: </b> {{ $booking->payment_method ?? 'N/A' }}</p>
              
            </div>
            <div class="col-xs-3">
                <p class="lead"><i class="fa fa-info-circle" aria-hidden="true"></i> Package Info:</p>
                <p><b>Package type: </b> {{ $booking->package_type ?? 'N/A' }}</p>

                <p><b>Package Instruction: </b> {{ $booking->booking_instruction ?? 'N/A' }}</p>
