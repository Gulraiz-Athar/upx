<!-- About Us -->
   <section class="pad-80 about-wrap clrbg-before leftRedStrip" >
      <span class="bg-text wow fadeInUp" data-wow-offset="50" data-wow-delay=".20s"> About </span>
      <div class="theme-container container">
         <div class="row">
            <div class="col-md-6">
               <div class="about-us">
                  <h4>UPX is a Courier and Cargo Company which offers a worldwide, door-door parcel courier and cargo service by Air, Road and Sea making us a one-stop shop for our customers. </h4>
                  <p class="fs-16 wow fadeInUp" data-wow-offset="50" data-wow-delay=".25s">UPX  is a Courier and Cargo Company which offers a worldwide, door-door parcel courier and cargo service by Air, Road and Sea. With our excellent working relationships with partners and agents throughout Europe, Africa, Asia, Americas and the Far East, we offer services by road, sea and air, making us truly a one-stop shop for our customers.
                     <p>
              
                  <ul class="feature">
                     <li>
                        <img alt="" src="{{ URL::asset('assets/img/icons/icon-2.png') }}" class="wow fadeInUp" data-wow-offset="50" data-wow-delay=".20s" /> 
                        <div class="feature-content wow rotateInDownRight" data-wow-offset="50" data-wow-delay=".30s">
                           <h2 class="title-1">Fast delivery</h2>
                           <p>We provide you fastest delivery with competitive prices</p>
                        </div>
                     </li>
                     <li>
                        <img alt="" src="{{ URL::asset('assets/img/icons/icon-3.png') }}" class="wow fadeInUp" data-wow-offset="50" data-wow-delay=".20s" /> 
                        <div class="feature-content wow rotateInDownRight" data-wow-offset="50" data-wow-delay=".30s">
                           <h2 class="title-1">secured service</h2>
                           <p>Proving a great service with Honesty & Integrity.</p>
                        </div>
                     </li>
                     <li>
                        <img alt="" src="{{ URL::asset('assets/img/icons/icon-4.png') }}" class="wow fadeInUp" data-wow-offset="50" data-wow-delay=".20s" /> 
                        <div class="feature-content wow rotateInDownRight" data-wow-offset="50" data-wow-delay=".30s">
                           <h2 class="title-1">worldwide shipping</h2>
                           <p>UPX provides fast worldwide courier service for all your parcels.</p>
                        </div>
                     </li>
                  </ul>
               </div>
            </div>
            <div class="col-md-6 text-center">
               <div class="pb-80 visible-lg"></div>
               <img alt="" src="{{ URL::asset('assets/img/block/about-img.png') }}" class="wow slideInRight" data-wow-offset="50" data-wow-delay=".20s" />
            </div>
         </div>
      </div>
   </section>
   <!-- /.About Us -->